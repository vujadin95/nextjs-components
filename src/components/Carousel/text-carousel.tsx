...
export default function TextCarousel({
  ...
  return (
    <div className="relative h-full w-full select-none overflow-hidden rounded-md">
      <div
        className="flex w-full justify-end rounded-md bg-transparent transition-transform duration-500 ease-in-out"
        style={{ transform: `translateX(${curr * 100}%)` }}
      >
        {slides.map((slide) => (
          <h2 key={slide.id} className="block w-full shrink-0">
            {slide.text}
          </h2>
        ))}
      </div>
          ...
          {slides.map((slide, index) => (
            <div
              ...
              className={cn(
                ...
                curr === index && "scale-125 bg-black dark:bg-white",
              )}
            />
          ))}
        </div>
